...
from anvil.config import DEFAULT_CONFIG_LOCATIONS, find_default_config, load_env_config


class ConfigTests(unittest.TestCase):
    def test_should_prefer_anvil_default_locations(self) -> None:
        self.assertEqual(DEFAULT_CONFIG_LOCATIONS[0], './anvil.yaml')
        self.assertTrue(all('anvil' in location for location in DEFAULT_CONFIG_LOCATIONS))

    def test_should_ignore_unknown_project_config_name(self) -> None:
        ...
        try:
            os.chdir(cwd)
            unknown = Path('agent.yaml')
            unknown.write_text('provider: mock\n', encoding='utf-8')
    ...
    def test_should_find_anvil_home_config(self) -> None:
